...
export class OutboxService {
  ...
  async sendDue(): Promise<{ sent: number; failed: number; leftAlone: number }> {
    ...
    const claimed = await this.db.client.commsRecipient.updateManyAndReturn({
      where: { id: { in: due.map((d) => d.id) }, status: 'PENDING' },
      data: { status: 'SENDING', attempts: { increment: 1 } },
    });
    const messageIds = [...new Set(claimed.map((r) => r.messageId))];
    await this.db.client.commsMessage.updateMany({
    ...
    const blocked = new Set(
      (
        await this.db.client.commsBlockedNumber.findMany({
  ...
  async pollDelivery(): Promise<{ delivered: number; undelivered: number }> {
    ...
    const rows = await this.db.client.commsRecipient.findMany({
      where: {
        status: 'SENT',
  ...
  private async finish(messageIds: string[]) {
    for (const id of messageIds) {
      const counts = await this.db.client.commsRecipient.groupBy({
        by: ['status'],
        where: { messageId: id },
        _count: { _all: true },
      });
  ...
  private async block(phone: string, reason: string, personId: string | null, note: string) {
    await this.db.client.commsBlockedNumber.upsert({
      where: { phone },
      update: {},
